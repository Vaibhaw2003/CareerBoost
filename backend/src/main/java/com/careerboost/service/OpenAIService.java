...
public class OpenAIService {
    ...
    /**
     * Smart demo mode: Generates realistic mock responses based on the feature requested.
     */
    private String generateDemoResponse(String systemPrompt, String userMessage) {
        ...
        // 4. Personal Branding
        if (promptLower.contains("branding") || promptLower.contains("linkedin post") || promptLower.contains("world-class personal branding")) {
            return "{" +
        ...
        skillCategories.put("Programming", List.of("java", "python", "javascript", "typescript", "c++", "c#", "go", "rust", "kotlin", "swift", "ruby", "php", "scala"));
        skillCategories.put("Frontend", List.of("react", "angular", "vue", "html", "css", "tailwind", "bootstrap", "next.js", "nextjs", "svelte"));
        skillCategories.put("Backend", List.of("spring", "node", "express", "django", "flask", "fastapi", ".net", "rails", "laravel"));
        skillCategories.put("Database", List.of("sql", "mongodb", "postgresql", "mysql", "redis", "elasticsearch", "dynamodb", "firebase"));
        skillCategories.put("Cloud", List.of("aws", "azure", "gcp", "docker", "kubernetes", "terraform", "jenkins", "ci/cd", "devops"));
        skillCategories.put("AI/ML", List.of("machine learning", "deep learning", "tensorflow", "pytorch", "nlp", "computer vision", "data science"));
        ...
        boolean hasExperience = textLower.contains("experience") || textLower.contains("work history") || textLower.contains("employment");
        boolean hasEducation = textLower.contains("education") || textLower.contains("degree") || textLower.contains("university") || textLower.contains("bachelor") || textLower.contains("master");
        boolean hasProjects = textLower.contains("project") || textLower.contains("portfolio");
        boolean hasCertifications = textLower.contains("certif") || textLower.contains("certified") || textLower.contains("certification");
        boolean hasContact = textLower.contains("email") || textLower.contains("phone") || textLower.contains("linkedin") || textLower.contains("github");
        boolean hasSummary = textLower.contains("summary") || textLower.contains("objective") || textLower.contains("about me") || textLower.contains("profile");
        boolean hasMetrics = textLower.contains("%") || textLower.contains("increased") || textLower.contains("reduced") || textLower.contains("improved") || textLower.contains("achieved");
        boolean hasActionVerbs = textLower.contains("developed") || textLower.contains("implemented") || textLower.contains("designed") || textLower.contains("managed") || textLower.contains("led") || textLower.contains("built");

        // ── Calculate score ──
        int score = 40; // Base score
        if (foundSkills.size() >= 5) score += 12;
        else if (foundSkills.size() >= 3) score += 8;
        else if (foundSkills.size() >= 1) score += 4;
        ...
        if (textLength > 500 && textLength < 4000) score += 2;
        if (categoryCount.size() >= 3) score += 5;
        ...
        if (foundSkills.size() >= 5) {
            strengths.add("Strong technical skill set with " + foundSkills.size() + " relevant technologies identified including " + String.join(", ", foundSkills.subList(0, Math.min(4, foundSkills.size()))));
        ...
        if (hasExperience) strengths.add("Professional experience section is present, which is essential for ATS systems");
        if (hasEducation) strengths.add("Educational background is clearly documented");
        if (hasProjects) strengths.add("Project portfolio demonstrates hands-on implementation ability");
        if (hasCertifications) strengths.add("Professional certifications add credibility and show continuous learning");
        if (hasMetrics) strengths.add("Quantifiable achievements help recruiters gauge impact (great use of metrics!)");
        if (hasActionVerbs) strengths.add("Good use of action verbs to describe accomplishments");
        if (hasSummary) strengths.add("Professional summary provides a quick snapshot for recruiters");
        ...
        if (!hasSummary) weaknesses.add("Missing a professional summary or objective section — recruiters spend only 7 seconds on initial scan");
        if (!hasMetrics) weaknesses.add("Lacks quantifiable metrics and numbers — use data like 'improved performance by 30%' to show impact");
        if (foundSkills.size() < 3) weaknesses.add("Limited technical keywords detected — ATS systems may not rank this resume highly");
        if (!hasProjects) weaknesses.add("No dedicated projects section — showcasing personal or open-source projects can differentiate you");
        if (!hasCertifications) weaknesses.add("No certifications mentioned — industry certifications can boost credibility");
        if (!hasContact) weaknesses.add("Contact information may be incomplete — ensure LinkedIn and GitHub profiles are included");
        if (textLength < 500) weaknesses.add("Resume appears too short — consider elaborating on experiences and skills");
        if (textLength > 4000) weaknesses.add("Resume may be too lengthy — try to keep it concise and within 1-2 pages");
        if (!hasActionVerbs) weaknesses.add("Consider using stronger action verbs like 'developed', 'implemented', 'designed' to describe achievements");
        ...
        if (!hasMetrics) suggestions.add("Add quantifiable achievements (e.g., 'Reduced API response time by 40%', 'Managed a team of 5 engineers')");
        if (!hasSummary) suggestions.add("Add a 2-3 line professional summary at the top highlighting your years of experience and key strengths");
        if (foundSkills.size() < 5) suggestions.add("Expand your technical skills section — include specific frameworks, tools, and technologies you've worked with");
        if (!hasProjects) suggestions.add("Add a projects section showcasing 2-3 impactful projects with tech stack and outcomes");
        suggestions.add("Use a clean, ATS-friendly format with standard section headings (Experience, Education, Skills, Projects)");
        if (!hasCertifications) suggestions.add("Consider adding relevant certifications (e.g., AWS Certified, Google Cloud, Oracle Java) to stand out");
        suggestions.add("Ensure consistent formatting — use the same date format, font sizes, and bullet styles throughout");
        if (categoryCount.size() < 2) suggestions.add("Diversify your skill set to show full-stack or cross-domain capability");
        ...
        if (isRoast) {
            ...
            if (score > 70) score -= 10; // Roast mode is harsher
